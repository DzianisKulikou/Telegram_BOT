...
async def process_positive_answer(message: Message):
    if not user['in_game']:
        await message.answer(f'Ура!\n\nЯ загадал число от 1 до 100, '
                             f'попробуй угадать! У тебя на это есть {ATTEMPTS} попыток!')
        user['in_game'] = True
...
async def process_numbers_answer(message: Message):
    if user['in_game']:
        if int(message.text) == user['secret_number']:
            ...
            user['attempts'] -= 1
            await message.answer(f'Мое число меньше, у вас осталось попыток:  {user["attempts"]}')
        elif int(message.text) < user['secret_number']:
            user['attempts'] -= 1
            await message.answer(f'Мое число больше, у вас осталось попыток:  {user["attempts"]}')
